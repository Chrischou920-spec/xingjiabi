...
// 12306 目前会把未登录的中转初始化页重定向到登录页，旧实现因此在
// 服务启动阶段直接崩溃。直接余票查询并不依赖该地址，所以启动时使用
// 官方长期使用的默认路径；仅中转查询需要它。我们的 MVP 中转由规划器
// 基于两个直达查询组合，不调用 get-interline-tickets。
const LCQUERY_PATH = '/lcquery/queryG';
